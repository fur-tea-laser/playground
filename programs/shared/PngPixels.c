...
Rgb8bitPixelChannels* atPixelsDataPixelChannels(
	Rgb8bitPngPixels* pngPixels, 
	U32 pixelColumnIndex, 
	U32 pixelRowIndex
) {
...
IEEE64 getPixelsWidthOverHeightAspectRatio(
	Rgb8bitPngPixels* pngPixels
) {
  return (IEEE64)pngPixels->width / pngPixels->height;
}

U32 getPixelsMinimumDimension(
	Rgb8bitPngPixels* pngPixels
) {
