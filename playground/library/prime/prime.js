 ...
 * ```
 */
export function prime(primeIndex) {
    return (primeSequence(primeIndex)[primeIndex] ?? throwInvalidPathError("prime"));
}
/**
 ...
 * ```
 */
export function primeSequence(maxPrimeIndex) {
 ...
 * ```
 */
export function primeSequenceInclusive(maxNumber) {
 ...
 * ```
 */
function numberGreaterThanPrime(primeIndex) {
    if (primeIndex === 0) {
        return 2 + 1;
    }
        ...
        return Math.floor(adjustedPrimeIndex * Math.log(adjustedPrimeIndex) +
 ...
 * ```
 */
export function primeSequenceInRange(minNumber, maxNumber) {
    const maxNumberSequence = primeSequenceInclusive(maxNumber);
    const minNumberIndex = maxNumberSequence.findIndex((somePrime) => somePrime >= minNumber);
