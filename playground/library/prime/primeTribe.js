 ...
 * ```
 */
export function primeTribe(tribeIndex) {
    const maxContainerSequence = primeContainerSequence(tribeIndex + 1);
    const tribeLowerBound = maxContainerSequence[maxContainerSequence.length - 2];
    const tribeUpperBound = maxContainerSequence[maxContainerSequence.length - 1];
    return [
        tribeLowerBound,
        tribeUpperBound,
 ...
 * ```
 */
export function tribeSpacer(tribeIndex) {
    const [tribeLowerBound, tribeUpperBound, tribeMembers] = primeTribe(tribeIndex);
    return [
        tribeUpperBound - tribeLowerBound,
        tribeMembers.map((someTribeMember) => someTribeMember - tribeLowerBound - 1),
    ];
}
