...
import { euclidSpacer } from "./euclidSpacer.js";
/**
 ...
 * ```
 */
export function spacer(someSpacerStructure) {
    const [spacerResolution, rootLayer, ...subLayers] = someSpacerStructure;
    const resultSpacer = euclidSpacer(spacerResolution, rootLayer[0], rootLayer[1], rootLayer[2] ?? 0);
    for (const currentSubLayer of subLayers) {
        const layerSpacer = euclidSpacer(resultSpacer[1].length, currentSubLayer[0], currentSubLayer[1], currentSubLayer[2] ?? 0);
        resultSpacer[1] = layerSpacer[1].map((someLayerPoint) => resultSpacer[1][someLayerPoint] ??
 ...
 * ```
 */
export function spacerId(someSpacerStructure) {
    const spacerType = someSpacerStructure[1].length === 2
        ? "aligned"
        : someSpacerStructure[1].length === 3
            ? "phased"
            : throwInvalidPathError("spacerId");
    const [spacerResolution, ...spacerLayers] = someSpacerStructure;
    return spacerLayers.reduce((resultId, someSpacerLayer) => `${resultId}__${someSpacerLayer.join("_")}`, `${spacerType}__${spacerResolution}`);
}
/**
 ...
 * ```
 */
export function spacerComponents(someSpacerStructure) {
    const [spacerResolution, ...spacerLayers] = someSpacerStructure;
    return spacerLayers.map((_, layerIndex) => [
        spacerResolution,
        ...spacerLayers
 ...
 * ```
 */
export function spacerLineage(someAlignedSpacerStructure) {
    const [spacerResolution, ...alignedSpacerLayers] = someAlignedSpacerStructure;
    return alignedSpacerLayers.map((_, layerIndex) => [
        [
            spacerResolution,
            ...alignedSpacerLayers
                .slice(0, layerIndex)
        ...
        alignedSpacerLayers
            .slice(layerIndex)
            .map((someAlignedSpacerLayer) => someAlignedSpacerLayer[0]),
    ]);
}
